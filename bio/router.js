...
app.get('/scan/getTxnEvent/:txHash', function (req, res) {
    ...
    connection.query(txnsql, contractAddress, (error, results, fields) => {
    ...
    connection.query(txnsql, [contractAddress], (error, results, fields) => {
        if (error) {
            ...
            if (req.params.toBlock == null || req.params.toBlock == undefined) {
                console.log('To Block entered valued is Null or undefined')
            }
            else {
                if (blockCount >= req.params.toBlock){
                    blockCount = req.params.toBlock
                }
                else if (blockCount < req.params.toBlock){
                    console.log('Block requested is out of range.')
                }
            }
            var contractTxn = []
...
app.post('/upload/abi', jsonParser, (req, res) => {

        let contract_name =  req.body.contract_name;
        let contract_abi = JSON.stringify(req.body.contract_abi);
        let contract_address = req.body.contract_address;
        let contract_txn = req.body.contract_txn;
        let contract_network = req.body.contract_network;
    ...
    var sql = "INSERT INTO contractabi.abi_address_map ( contract_name, contract_abi, contract_address, contract_txn, contract_network) \
               VALUES ('" + contract_name+"','"+ contract_abi+"','"+ contract_address+"','"+ contract_txn+"','"+ contract_network+"')";
    console.log('SQL: '+sql);
